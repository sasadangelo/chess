...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        default="",
        required=True,
        help="The user for whom the report is to be created",
    )
    parser.add_argument(
        "--num-games",
        type=int,
        default=-1,
        required=True,
        help="Number of recent games to select",
    )
    parser.add_argument(
        "--time-control",
        type=str,
        default="",
        required=True,
        help="The desired chess game time control (daily, rapid, blitz, bullet, etc.)",
    )
    parser.add_argument(
        "--output",
        type=str,
    ...
    parser.add_argument(
        ...
        default="",
        required=False,
        help="The pieces color (white, black, or any)",
    )
    ...
    if game_collection.total_games != 0:
        ...
        with open(args.output, "w") as report_file:
            report_file.write(
                f"# Chess games Report for the latest {args.num_games} {args.user}'s games.\n\n"
            )
            ...
            for opening_name, games in game_collection.games_by_opening.items():
                ...
                for game in games:
                    if (
                        (args.color == "")
                        or (args.color == "white" and game.white_player == args.user)
                        or (args.color == "black" and game.black_player == args.user)
                    ):
                        ...
                        report_file.write(
                            f"| [{game.white_player} ({game.white_elo}) vs {game.black_player} ({game.black_elo})]({game.link}) | {game.start_time.strftime("%Y%m%d %H:%M")} | [{url_text}]({game.opening_url}) | {image_markdown} |\n"
                        )
